...
import { getFormResponseDiagnostics, reportClientFormError } from './form/client-error-report';
import { leadAttempt, finishLeadAttempt } from './form/lead-attempt';

export { getFormResponseDiagnostics, reportClientFormError };
...
export const phoneChecker = (phone, options = {}) => {
	...
	if (!phoneRe.test(phone.value) || noValidPhone(phone.value)) {
		if (!silent && form) {
			showErrorMes(form, ".phone", "Укажите номер телефона в формате +7 999 999-99-99");
		}
		return false;
	}
...
const reachFormErrorGoal = (form, errorSource, errorStage, httpStatus, diagnostics = {}) => {
	...
	void reportClientFormError({ ...eventProperties, ...diagnostics });
};
...
const submitForm = async (form) => {
	...
	const attempt = await leadAttempt(form, url, formData, () => attemptCalltouchCallback({
	...
	if (attempt.requestId) formData.set('request_id', attempt.requestId);
	...
	try {
		...
		reachFormErrorGoal(form, 'server', 'response_parse', response.status, getFormResponseDiagnostics({
			responseText,
			response,
			url,
		}));
...
document.querySelectorAll("form:not(.vue-form)").forEach((form) => {
	// Native HTML5 validation blocks the submit event before our shared
	// validator can render inline errors and emit form_required.
