...
// Поля, которые нельзя подменять через параметры URL/cookie:
// контактные данные лида, маршрутизация (dealer → 1С, departament, email_recipient),
// служебные поля скрипта и прототипные ключи. Остальные параметры проходят как раньше.
const blockedUrlParamsDefault = [
	'phone', 'email', 'name', 'comment', 'agree',
	'dealer', 'departament', 'email_recipient',
	'page_url', 're', 'fta', 'ct_callback', 'ctw_createRequest', 'ct_mod_id',
	'__proto__', 'constructor', 'prototype',
];

const appendUrlParams = (formData, query, blockedExtra) => {
	const blocked = blockedUrlParamsDefault.concat(Array.isArray(blockedExtra) ? blockedExtra : []);
	...
	await fetch(url, requestOptions)
		...
		.then((data) => {
			...
			if (data.answer == "required") {
				...
				if (isAttentionFlag && !isAttentionTestPhone) {
