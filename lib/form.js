...
const submitForm = async (form) => {
	...
	// Если передана функция валидации (например, класс FormsValidation), создаём её экземпляр
	if (props.validation && typeof props.validation === 'function') {
		validate = new props.validation(form);
	} else {
		// Иначе проверяем телефон и дилерский центр, если он есть и обязателен
	...
	// если флажок установлен - устанавливаем куки
	if (agree.checked) {
		setAgreeCookie(90);
	}
