export function getCookie(name)
{
    let matches = document.cookie.match(new RegExp(
        "(?:^|; )" + name.replace(/([\.$?*|{}\(\)\[\]\\\/\+^])/g, '\\$1') + "=([^;]*)"
        ));
    if (!matches) return undefined
    // кривой %-эскейп в значении не должен ронять чтение куки
    try {
        return decodeURIComponent(matches[1])
    } catch (e) {
        return matches[1]
    }
}

export function setCookie(name, value, props)
{
    props = props || {}
    var exp = props.expires
    if (typeof exp == "number" && exp) {
        var d = new Date()
        d.setTime(d.getTime() + exp*1000)
        exp = props.expires = d
    }
    if(exp && exp.toUTCString) { props.expires = exp.toUTCString() }
    value = encodeURIComponent(value)
    var updatedCookie = name + "=" + value
    var hasSameSite = false
    var hasSecure = false
    for(var propName in props){
        if (propName.toLowerCase() == 'samesite') { hasSameSite = true }
        if (propName.toLowerCase() == 'secure') { hasSecure = true }
        updatedCookie += "; " + propName
        var propValue = props[propName]
        if(propValue !== true){ updatedCookie += "=" + propValue }
    }
    if (!hasSameSite) { updatedCookie += "; samesite=lax" }
    if (!hasSecure && window.location.protocol == 'https:') { updatedCookie += "; secure" }
    document.cookie = updatedCookie
}

export function deleteCookie(name)
{
    setCookie(name, null, { 'domain':window.location.hostname,'path':'/','expires': -1 })
}

export function cookiecook(days = 90)
{
    let cookiecook = getCookie("cookiecook"),
        cookiewin = document.querySelector('.cookie_notice');

    if (cookiecook != "yes") {

        cookiewin.classList.remove("hidden");

        document.getElementById("cookie_close").addEventListener("click", function(e){
            e.preventDefault();
            cookiewin.classList.add("hidden");

            setAgreeCookie(days);
        });
    }
}

export function setAgreeCookie(days = 90) {
    let date = new Date;
    date.setDate(date.getDate() + days);

    setCookie("cookiecook", "yes", { 'domain':window.location.hostname,'path':'/','expires': date.toUTCString() })
}