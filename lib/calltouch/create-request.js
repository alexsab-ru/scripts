...
// The package manifest is the single source of truth for the wire version.
// Consumers can therefore correlate legacy requests with the exact bundle
// without maintaining a second version literal in source code.
export const CALLTOUCH_CLIENT_VERSION = packageMetadata.version;
...
/**
 * Технический сбой — единственный статус, по которому сервер повторяет попытку.
 * Незнакомый код к нему не приравнивается: новый код со стороны Calltouch
 * гораздо вероятнее окажется очередным бизнес-отказом (лимит, блэклист,
 * выключенный виджет), чем сбоем транспорта. Серверный ретрай на таком коде
 * впустую сожжёт секунды ожидания, а в случае лимита ещё и обойдёт правило,
 * которое мы просто не распознали. Поэтому 'unknown': заявка доставляется
 * обычным Lead API, но повторная попытка звонка не делается.
 */
...
export const appendCalltouchResultToFormData = (formData, result) => {
	...
	const fields = {
		...
		ct_client_version: CALLTOUCH_CLIENT_VERSION,
	};
